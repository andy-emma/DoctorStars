...
bot.on('message', async message => {
    ...
    if (!isNaN(messages)) {
        ...
        let levelembed = new MessageEmbed()
            .setColor('GREEN')
            .setDescription(`**${message.author}, Tu subiste al lv ${levelfetch}**`)
            .setFooter(`${prefix}Disablexp Para deshabilitar los mensajes de subida de nivel`)
...
bot.on("guildMemberAdd", async member => {
  ...
  if (chx === null) {
   ...
   let data = await canva.welcome(member, { link: "welcome2.jpg" })
  ...
  bot.channels.cache.get(chx).send("¡Bienvenido al servidor!"  
   + `<@${member.user.id}>`, attachment);
...
bot.on("guildCreate", guild => {
  ...
  const embed = new MessageEmbed()
    ...
    .addField(`**Servidor ID:**`, `\`\`\`${guild.id}\`\`\``)

    .addField(`**Creador del servidor:**`, `\`\`\`${sowner.tag}\`\`\``)

    .addField(`**Creador ID:**`, `\`\`\`${sowner.id}\`\`\``)
 
    .addField(`**Creado en:**`, `\`\`\`${guild.createdAt}\`\`\``)
    ...
    .setFooter(`Conteo de servidores - ${client.guilds.cache.size}`);
...
bot.on("guildDelete", guild => {
  ...
  const embed = new MessageEmbed()
    ...
    .setTitle("**¡Me sali de un  servidor!**")
...
app.listen(3000, () => console.log(`El bot funciona xd`));
...
bot.login("ODkxMzg0ODU3MTAyNTQwODMw.YU9k1w._-WF9CvjjulFv2Iwh48XMCzWSXo");
