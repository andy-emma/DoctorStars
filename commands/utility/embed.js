...
exports.run = async (client, msg, args) => {
  if (!msg.member.hasPermission("ADMINISTRATOR"))
    return msg.channel
      .send("No tiene el permiso necesario para utilizar este comando.")
      .then(m => {
        setTimeout(() => {
          m.delete();
        }, 3000);
  ...
  msg
    .reply("¿Cuál debería ser el título de la inserción? Si no hay ninguno, escriba `none`")
  ...
  if (title.size) {
    if (title.first().content !== "none") {
      if (title.first().length > 256)
        return msg
          .reply("La descripción no puede superar los 2048 caracteres.")
