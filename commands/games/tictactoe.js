        ...
        try {
            ...
            while (!winner && taken.length < 9) {
                ...
                await message.channel.send(stripIndents`
					**${user}, ¿De qué lado eliges? Escribe \`End\` **
					\`\`\`
                ...
                if (!turn.size) {
                    await message.channel.send(`**¡Se acabó el tiempo, se acabó el juego!**`);
                    if (lastTurnTimeout) {
										  winner = 'time';
						          break;
					          } else {
            ...
            return message.channel.send(winner ? `**¡Felicidades, ${winner}!**` : '**¡Es un empate!**');
        } catch (err) {
            ops.games.delete(message.channel.id);
            throw err;
        }
