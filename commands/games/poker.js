...
module.exports = {
    ...
    run: async (bot, message, args, ops) => {
        ...
        if (!playersCount) return message.channel.send("**Por favor ingrese el número de jugadores que juegan, ¡excluyéndolo a usted!**")
        if (playersCount < 1 || playersCount >= 6) return message.channel.send("**¡Ingrese un número entre 1 y 5!**")
        ...
        try {
            ...
            if (awaitedPlayers.length - 1 < playersCount) {
                ...
                return message.channel.send('**¡No se unió una cantidad suficiente de jugadores!**');
            }
            ...
            while (!winner) {
                ...
                rotation.shift();
                const folded = [];
                await message.channel.send('**Repartir manos de jugador...**');
                for (const player of players.values()) {
                    ...
                    try {
                        await player.user.send(stripIndents`
							...
							**Dinero -** $${formatNumber(player.money)}
							${bigBlind.id === player.id ? '_¡Eres el gran ciego!_' : ''}
							${smallBlind.id === player.id ? '_¡Eres el pequeño ciego!_' : ''}
						`);
                    } catch (e) {
                        console.log(e);
                        await message.channel.send(`**${player.user}, ¡No pude enviar tu mano! ¡Encienda los DM!**`);
                    }
                }
                ...
                await message.channel.send(stripIndents`
					**Mano del distribuidor -**
					${dealerHand.map(card => card.textDisplay).join('\n')}
					_¡La próxima ronda de apuestas comienza en 5 segundos!_
				`);
                await delay(5000);
                ...
                if (winners.length > 1) {
                    await message.channel.send(stripIndents`
						La olla se dividirá entre${list(winners.map(w => `**${w.user.user}**`))}.
						${winners.map(winner.descr).join(', ')}
						__**Resultados**__
						${solved.map(solve => `${solve.user.user.tag}: ${solve.descr}`).join('\n')}
						_El próximo juego comienza en 10 segundos._
					`);
                    const splitPot = turnData.pot / winners.length;
                    for (const win of winners) win.user.money += splitPot;
                } else {
                    await message.channel.send(stripIndents`
						${winners[0].user.user} Toma la olla, con **${winners[0].descr}**.
						__**Resultados**__
						${solved.map(solve => `${solve.user.user.tag}: ${solve.descr}`).join('\n')}
						_¡El próximo juego comienza en 10 segundos!_
					`);
            ...
            ops.games.delete(message.channel.id);
            return message.channel.send(`**Felicitaciones, ${winner.user}!**`);
        } catch (err) {
            ops.games.delete(message.channel.id);
            throw err;
        }
        ...
        async function awaitPlayers(message, players) {
            await message.channel.send(`**Escribe \`join game\` Para unirte al juego.**`);
            const joined = [];
        ...
        async function gameRound(message, players, folded, turnData, bigBlind, smallBlind) {
            ...
            if (turnRotation.length === 1) {
                ...
                await message.channel.send(stripIndents`
				**${remainer.user} ¡Se lleva la olla!**
				_¡El próximo juego comienza en 10 segundos!_
			`);
                await message.channel.send(`**${remainer.user} ¡Se lleva la olla!**`);
        ...
        async function bettingRound(message, players, turnRotation, folded, data) {
			...
			**Pot - $${formatNumber(data.pot)}**
			_Apuesta más alta - $${formatNumber(data.currentBet)} (${data.highestBetter.user.tag})_
			${turnPlayer.user}, ¿Qué quieres hacer? Usted puede ${displayActions}.
		`);
            const filter = res => {
                ...
                if (choice.startsWith('raise')) {
                    ...
                    if (amount + data.currentBet > turnPlayer.money || amount < 1) {
                        ...
                        res.channel.send(`**Solo puedes apostar hasta $${formatNumber(maxBet)}!**`).catch(() => null);
                        return false;
                    }
                    return true;
                }
            ...
            if (raiseValue) {
                ...
                await message.channel.send(`**${turnPlayer.user} Eleva $${formatNumber(raiseValue)}!**`);
                ...
                await message.channel.send(`**${turnPlayer.user} Check!**`);
            }
        ...
        async function resetGame(message, players) {
            for (const player of players.values()) {
                if (player.money <= 0) {
                    await message.channel.send(`**${player.user} ¡Ha sido pateado por no tener dinero!**`);
                    players.delete(player.id);
                } else if (player.strikes >= 2) {
                    await message.channel.send(`**${player.user} ¡Ha sido pateado por no jugar!**`);
                    players.delete(player.id);
                } else {
                    player.currentBet = 0;
                    player.hand = [];
