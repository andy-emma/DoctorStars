  ...
  run: async (bot, message, args) => {
    ...
    if (author !== null && timeout - (Date.now() - author) > 0) {
      ...
      let timeEmbed = new MessageEmbed()
        .setColor("GREEN")
        .setDescription(`❌ Ya has robado a alguien\n\nInténtalo de nuevo en ${time.minutes}m ${time.seconds}s `);
      message.channel.send(timeEmbed)
    } else {

      let moneyEmbed = new MessageEmbed()
        .setColor("GREEN")
        .setDescription(`❌ Necesita al menos 100 monedas en su billetera para robar a alguien`);
      ...
      if (targetuser < random) {
        ...
        let embed = new MessageEmbed()
          .setDescription(`✅ Tu robaste ${user.user.username} y se salió con la suya ${random} monedas`)
          .setColor("GREEN")
